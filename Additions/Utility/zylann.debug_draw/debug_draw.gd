...
static var _running_singleton : DebugDraw
...
static func _check_singleton_created_or_create():
	if Engine.is_editor_hint() and not _editor_singleton:
		var debug_draw = DebugDraw.new()
		var editor_interface = Engine.get_singleton("EditorInterface")
		var root_node = editor_interface.get_edited_scene_root().get_parent()
		root_node.add_child.call_deferred(debug_draw)
		_editor_singleton = debug_draw
	elif not Engine.is_editor_hint() and not _running_singleton:
		var debug_draw = DebugDraw.new()
		var root_node = Engine.get_main_loop().current_scene.get_parent()
		root_node.add_child.call_deferred(debug_draw)
		_running_singleton = debug_draw
static func _get_singleton() -> DebugDraw:
	return _editor_singleton if Engine.is_editor_hint() else _running_singleton
...
static func _process_lines_2d():
	...
	# 		_get_singleton()._canvas_item.add_child(geometry)
	...
	# 		_get_singleton()._canvas_item.remove_child(line[4])
	# 		line[4].queue_free()
	# 	else:
	# 		i += 1
