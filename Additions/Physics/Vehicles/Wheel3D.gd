...
func _process(_delta: float) -> void:
	...
	var wheel_dist: float = min(_current_ground_dist, suspension_rest_dist)
	if debug:
		DebugDraw.draw_circle_3d(global_position - wheel_up * (wheel_dist - wheel_radius), global_basis.rotated(wheel_up, (PI / 2) + _current_angle).get_rotation_quaternion(), wheel_radius)
	...
	if wheel_visual:
		wheel_visual.position = Vector3.DOWN * (wheel_dist - wheel_radius)
...
func calculate_force_on_vehicle(car: Car, input_vector: Vector2, delta: float) -> Vector3:
	...
	if _raycast.is_colliding():
		...
		_current_angle = calculate_steer_angle(input_vector.x)
